...
""".strip(),
    ...
    def test_connected_six_key_deck_reports_hardware_and_keeps_hidden_buttons(self):
        ...
        stored["streamdeck"]["buttons"] = {
            14: {"enabled": True, "label": "LARGER DECK", "action_type": "none"}
        }
        self.config_path.write_text(yaml.safe_dump(stored), encoding="utf-8")
        configure_config_ui(
            self.config_path,
            self.restart,
            streamdeck_layout_provider=lambda: (2, 3),
        )
        config = get_configuration()
        self.assertEqual(config["streamdeck"]["key_count"], 6)
    ...
    def test_ha_toggle_rejects_unscoped_services(self):
        from pydantic import ValidationError

        with self.assertRaises(ValidationError):
            StreamDeckButtonSettings.model_validate(
                {
                    "key": 2,
    ...
    def test_get_system_version(self):
        ...
        with (
            patch(
                ...
                return_value={"latest_version": "v1.2.0", "update_available": False},
            ),
            patch.object(
                source_version,
                "RUNNING_SOURCE",
                {"commit": revision, "dirty": False, "provenance": "deployed source"},
            ),
            patch.object(
                source_version,
                "github_head",
            ...
            self.assertEqual(res["current_commit"], revision)
            self.assertEqual(res["latest_commit"], revision)
            self.assertEqual(res["source_status"], "current")

    def test_version_check_failure_is_not_reported_as_up_to_date(self):
        from unittest.mock import patch
        ...
        with (
            ...
            patch.object(
                source_version,
                "github_head",
                return_value={"commit": None, "url": None},
            ),
        ):
            res = get_system_version()
